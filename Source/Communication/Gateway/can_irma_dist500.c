...
/*----- Global Variables -----------------------------------------------------------------------*/
bool use_static_can_sensor_addrsB;
byte can_sensor_commu_step;
byte can_sensor_commu_idx;
bool can_sensor_commu_cycle_failedB;
byte can_sensor_regular_commu_step;
bool extended_sensor_commu_requiredB;
bool first_can_sensor_ext_commu_stepB;
bool can_sensor_static_data_changedB;

word can_sensor_commu_cycle_start_delay;
dword can_sensor_commu_cycle_cnt;
dword can_sensor_commu_cycle_error_cnt;
bool first_can_sensor_commu_cycle_startedB;
...
bool update_can_sensor_list(void);
...
bool update_static_can_sensor_data(void);
bool get_is_gateway_sensor_flag(byte sen_idx);
...
bool update_counting_data_category_params(void);

bool sensor_door_state_setting_confirmed(void);
bool sensor_door_closing_before_reset_confirmed(void);
bool sensor_status_received(void);
bool sensor_fa_status_received(void);
bool sensor_counting_data_received(void);
bool sensor_counting_data_set_confirmed(void);
bool sensor_counting_result_reset_confirmed(void);
bool sensor_firmware_reset_confirmed(void);
...
bool to_query_counting_data_category_params(can_addr_id_type sensor_addr_id)
{
...
void send_C_60_s_to_sensor(can_addr_id_type sensor_addr_id, char type_of_counting_data, bool counting_finishedB)
{
...
void continue_can_sensor_communication(void)
{
	...
	bool static_data_knownB, is_gateway_sensorB, new_counting_result_availableB;
	bool sensor_firmware_reset_requiredB;
...
bool sensor_found_in_list(dword sensor_addr)
{
	byte sen_idx;
...
{
	bool communication_cycle_finishedB = FALSE;
...
{
	dword sensor_addr;
	bool resultB, sensor_addr_knownB;
	byte sen_idx;
	word payload_len = 0;
	byte *payload = 0;
...
bool get_sensor_conf_data_str(char *des_ptr, byte *payload)
{
	word str_len;
...
bool get_sensor_conf_data_byte(byte *byte_ptr, byte *payload)
{
	word data_len;
...
bool get_sensor_conf_data_word(word *word_ptr, byte *payload)
{
	word data_len;
...
{
	dword sensor_addr;
	bool resultB, error_msg_receivedB = FALSE;
	word payload_len = 0;
	byte *payload = 0;
...
/*
bool addr_found_in_fa_addrs(byte function_area_cnt, word function_area_addr)
{
	bool foundB = FALSE;
	byte fa_idx;
...
bool no_found_in_door_signal_nos(byte door_cnt, word door_signal_no)
{
	bool foundB = FALSE;
	byte door_idx;
...
bool check_uip_addrs_of_function_area(byte *sen_cnt, sensor_assignments_type *sensor_assignments_ptr, uip_address_list_type *uip_address_list_ptr)
{
...
bool get_sensor_to_function_area_assignments(byte function_area_addr, byte *sen_cnt, sensor_assignments_type *sensor_assignments_ptr, bool *static_data_of_all_assigned_sensors_knownB)
{
	byte sen_idx;
	bool resultB;
...
bool check_validity_of_static_data_of_assigned_sensors(byte fa_idx)
{
...
void update_function_area_and_door_list(void)
{
	...
	sensor_assignments_type *sensor_assignments_ptr;
	bool resultB = FALSE, static_data_of_all_assigned_sensors_knownB, new_doorB;
...
bool sensor_door_state_setting_confirmed(void)
{
	...
	byte left_wing_opening_degree, right_wing_opening_degree;
	bool closedB = FALSE, closed_oldB = FALSE;
...
{
	dword sensor_addr;
	bool resultB, is_gateway_sensorB, error_msg_receivedB = FALSE;
	word payload_len = 0;
	byte *payload = 0;
	word function_area_addr;
	bool reset_flagB;
...
bool sensor_counting_data_set_confirmed(void)
{
	...
	if( resultB && !error_msg_receivedB )
	{
		function_area_addr = (word)*(payload++) + ((word)*(payload++) << 8);
		type_of_counting_data = (char)*(payload++);
		counting_finishedB = (bool)*(payload++);
		counting_data_category_cnt = (byte)*(payload++);
		resultB = payload_len == 5 + 5 * counting_data_category_cnt;
...
void update_fa_list_of_obc_interface_data(void)
{
	...
	word fa_addr;
	bool fa_addr_knownB;
	bool door_closedB, door_closed_oldB;
...
void update_obc_interface_data_backup(void)
{
	...
	dword sensor_addr;
	bool sensor_addr_knownB, fa_addr_knownB;
	word fa_addr;
