...
volatile bool Door_Status_Synchronized;
...
// Refer to comments regarding pragma volatile in source file "serio_asc0.c".
volatile bool First_Doors_CheckingB;
...
volatile bool Send_J1708_Msgs_On_Door_Status_ChangeB;
#endif

#ifdef GLORIA
bool GLORIA_InitializedB;
	#ifdef DEVTEST
bool GLORIA_BurnIn_ModeB;
	#endif
#endif
...
void clear_comm_RAM(void)
{
	...
	p = (unsigned int huge *)ADDR_COMMRAM;
	while( p <= (unsigned int huge *)ADDR_COMMRAM_END )
		*(p++) = 0;
}
...
bool add_door_opened_job(void huge *ptr)
{
	byte n = 0;
...
bool add_door_closed_job(void huge *ptr)
{
	byte n = 0;
...
bool remove_door_opened_job(void huge *ptr)
{
	byte n = 0;
	byte m;
...
bool remove_door_closed_job(void huge *ptr)
{
	byte n = 0;
	byte m;
