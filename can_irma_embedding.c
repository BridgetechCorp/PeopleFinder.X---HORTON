...
boolean uip_30_rec_buf_validB;

boolean to_enable_can_transparent_modeB;
boolean can_transparent_modeB;
boolean (*enable_can_transparent_mode_func)(void);
...
boolean Muzzle_ModeB;

boolean to_update_can_sensor_commu_idx_and_stepB;
boolean to_delay_can_sensor_commuB;
boolean to_continue_can_sensor_commuB;
...
/*----- Function Prototypes --------------------------------------------------------------------*/
boolean start_can_sensor_direct_query_of_devimmanent_confdata(void);
...
//|-------------------------------------------------------------------------------------|
void send_can_irma_msg_set(byte *data_ptr, boolean complete_msg_noB, byte total_length,
                           byte can_irma_msg_id, byte rec_class, can_addr_id_type addr_id)
{
	byte id_ll  = (can_irma_msg_id << 1) + (rec_class >> 2);
	byte id_lh  = (rec_class << 6) + addr_id.high;
...
boolean check_rec_uip_30_msg(byte service_lev, char msg_id, byte msg_ver, char sub_cmd, word *payload_len, byte **payload)
{
	boolean resultB;
...
boolean set_can_transparent_mode_enable_flag(void)
{
	if( !can_transparent_modeB )
...
boolean enable_can_transparent_mode(void)
{
	boolean can_transparent_mode_oldB;
...
boolean disable_can_transparent_mode(void)
{
	boolean can_transparent_mode_oldB;
