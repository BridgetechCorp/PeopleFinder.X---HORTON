...
boolean obc_interface_data_fa_cnt_increasedB, obc_interface_data_fa_cnt_decreasedB;
...
boolean startup_communication_prot_prepared(void);
...
{
	boolean person_in_1 = FALSE, person_out_1 = FALSE, person_in_2 = FALSE, person_out_2 = FALSE;
...
{
	boolean person_in = FALSE, person_out = FALSE;
...
#else
int run_application_module(boolean Logger_OKB)
#endif
{
	int rlv_ret;
...
int communication_RL4(void)
{
	boolean runlevel5_desiredB;
	boolean to_init_startup_communication_protB;
#ifndef NO_LOGGER
	//boolean Logger_Memory_OKB;
	//boolean Logger_OKB;
#endif
//	int rlv_ret;
...
boolean door_contacts_ever_changed_gateway(void)
{
	byte p2_channel_idx;
...
boolean get_logical_p2_channel_state(byte door_signal_idx)
{
	byte p2_channel_no;
...
#ifdef DOOR_LOG_POS
   	door_closedB = (boolean)p2_channel_state;		// logically positive
#else
	if( door_signal_interface.positive_logicB[door_signal_idx] )
	   	door_closedB =  (boolean)p2_channel_state;	// logically positive
	else
	   	door_closedB = !(boolean)p2_channel_state;	// logically negative
#endif
...
{
	boolean current_input_stateB, to_use_current_input_stateB = TRUE;
...
void update_desired_function_area_data(void)
{
	...
	boolean to_send_responseB = FALSE;
...
boolean get_new_counting_result(byte fa_idx, counting_data_buffer_type *counting_data_buffer)
{
