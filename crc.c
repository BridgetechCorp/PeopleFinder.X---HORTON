...
/*----- Global Constants -----------------------------------------------------------------------*/
const unsigned long crc32table[] = { /* CRC polynomial 0xedb88320 */
...
unsigned long Get_CRC(unsigned char  *start, unsigned char  *end)
{
	unsigned long crc;
//	unsigned long i;		// Für Variante 1 und 2

	crc = 0xFFFFFFFF;
	while(start <= end)
	{
//		i = crc32table[(crc ^ *start) ^ 0xFFUL];		// Variante 1
//		crc = ((crc >> 8) & 0x00FFFFFF) ^ i;
//
//		i = crc32table[*start ^ (crc & 0x000000FFUL)];	// Variante 2
//		crc = (crc >> 8) & i;
//
		crc = ((crc >> 8) & 0x00FFFFFFUL) ^ crc32table[(crc ^ (unsigned long)*start) & 0xFFUL];
		start++;
	}
