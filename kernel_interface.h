...
extern boolean ASC0_Service_Interface_OccupiedB;
extern boolean ASC0_Debug_ModeB;
...
boolean ASC0_baudrate_valid(dword desired_baudrate);
word InitASC0(RS232_Parameter_type *parameter, byte prot_id);
boolean j1708_prot_active(void);
...
boolean WaitForEndOfTransmission(word Release_TO_ms, word Flush_TO_ms);
void Clear_TerminalSymbol(void);

void Change_ASC0_Debug_Mode(boolean valueB);

void Update_ASC0TimeoutRef(void);
boolean ASC0TimeoutCond(void);
boolean ASC0_Transmission_Finished(void);


/*----- time.c ---------------------------------------------------------------------------------*/
extern volatile boolean timer_and_timeout_jobs_disabledB;
...
void Set_RTC_Sync_Status(boolean);
boolean Get_RTC_Sync_Status(void);

int Get_Date_Time(Date_type *, Time_type *);
boolean Set_Date_Time(Date_type *, Time_type *, Date_type *, Time_type *);

void add_clock_job(word, void *);
void remove_clock_job(void *);

word add_timer_job(word, void *);
boolean remove_timer_job(void *);

word add_timeout_job(word ms, void *);
word reload_timeout_job(void *);
boolean remove_timeout_job(void *);

boolean Get_Timeout_Flag(void);
word Add_Def_Timeout_Job(word ms);
boolean Remove_Def_Timeout_Job(void);

word StartTimeOutIntv(word TO_ms);
boolean TimeOutIntvElapsed(void);
...
void get_firmware_name(boolean get_inherent_firmware_nameB, byte *res_buff, byte *res_buff_pos);
boolean is_inherent_firmware_name(void);

void Set_Runlevel(byte rlv);
byte Get_Runlevel(void);
boolean Runlevel_4_or_5(void);
...
boolean SectorValid(byte sector);
...
extern boolean Reset_Request_ReceivedB;

extern boolean Recovery_Mode;

#if defined(CAN_SENSOR) || defined(GATEWAY)
extern byte can_sensor_init_status;
#endif

boolean Add_Runlevel_Job(void *ptr);
boolean Remove_Runlevel_Job(void *ptr);
...
extern boolean ChannelState_ChangedB[A21_Digital_IO_Cnt];

char Get_Channel(byte chan_no);
#ifndef NO_LOGGER
boolean RTC_Error(void);
boolean Logger_Memory_Error(void);
void Set_Logger_Data_Status(boolean);
boolean Logger_Empty(void);
void Set_Logger_Overflow_Status(boolean);
boolean Logger_Overflow(void);
#endif


/*----- flash.c --------------------------------------------------------------------------------*/
boolean FlashProg_Allowed(dword MemAddr);
...
extern volatile boolean SensorData_readyB;
extern volatile SensorStatus_type SensorStatus[SENSORS_MAX];
extern volatile boolean initial_sensor_commutestB;

void Init_Signal_Buffer(void);
void Set_Signal_Buffer_Overflow(bool);
boolean Signal_Buffer_Overflow(void);
#endif


/*----- configuration.c ------------------------------------------------------------------------*/
extern dword fCPU_div_32;
extern boolean fCPU_div_32_validB;
extern byte MaxFACnt;
extern byte MaxSensorCnt;
extern boolean Height_Classification_SupportedB;
...
boolean Rigth_Configuration_Version(void);
...
boolean DataFlash_Read(dword, word, byte *);
boolean DataFlash_Write(dword, word, byte *);
...
boolean Valid_Logger_Address(dword MemAddr);
boolean Valid_Logger_Range(dword MemAddr, dword MemAddrEnd);
boolean Logger_Write_Allowed(dword MemAddr);
...
{
#ifdef GATEWAY
	void (*call_function)(byte, byte, boolean, byte, byte, char, word, byte *);
#else
...
extern boolean IRMA_Frame_ReceivedB;
...
extern boolean uip_retry_flagB;
#endif
...
#ifdef GATEWAY
extern boolean is_uip_20_sendB;
...
#ifndef GLORIA
extern boolean Routing_To_SysInterf_ActiveB;
#endif
...
#ifdef DEVTEST
	#ifdef GLORIA
extern boolean Resp_to_EmbMsg_ExpectedB;
extern int  EmbMsgDI;
...
boolean get_char_asc0_uip(void);
...
boolean Query_FAIdx_ValidB(byte PayloadLen, byte *Payload, byte *FAIdx);
#ifndef DEVTEST
boolean Query_HCLims_ValidB(byte *Payload, byte *HCIdx);
#endif
