...
/*----- Global Variables -----------------------------------------------------------------------*/
boolean to_route_uip_30_msg_to_canB;

boolean count_data_statussesB[CAN_FA_LIST_LEN];
...
void irma_msg_embedding_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_baudrate_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_count_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_door_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_error_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_flags_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_information_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#ifndef NO_LOGGER
void irma_msg_logger_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#endif
void irma_msg_memory_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_new_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_port_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#ifdef TEST
void irma_msg_query_sensor_signal_test_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#endif
void irma_msg_programm_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_run_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_status_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#ifndef NO_LOGGER
void irma_msg_time_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#endif
void irma_msg_runlevel_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_version_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
void irma_msg_execute_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#ifdef TEST
void irma_msg_inst_mode_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload);
#endif
...
{
	boolean can_transparent_mode_just_enabledB;
...
boolean prepare_can_transparent_mode_enabling(void)
{
	boolean can_transparent_mode_enable_flag_setB;
...
byte IRMA_A_v60_d_e_Resp(boolean to_enableB, word PayloadLen, byte *Payload, boolean *to_respond_immediatelyB)
{
	byte ErrVal;
...
byte IRMA_A_v60_s_Resp(word PayloadLen, byte *Payload, boolean *to_respond_immediatelyB)
{
	byte ErrVal;
...
{
    byte Result;
	boolean to_respond_immediatelyB = TRUE;
...
/*---------------------------------------------------------------------------------------- C ---*/
boolean query_fa_idx_valid_gatewayB(byte payload_len, byte *payload, byte *fa_idx)
{
	byte fa_addr;
		...
		if( PayloadLen >= C_50_TYPE_SIZE && PayloadLen % C_50_TYPE_SIZE == 0 )
		{
			...
			if( FACnt > 0 && PayloadLen == FACnt * C_50_TYPE_SIZE )
			{
				...
				for( FAIdx = 0; FAIdx < FACnt; FAIdx++ )
				{
					...
					counting_finishedB = (boolean)Payload[PayloadPos + 1];
...
byte irma_D_v10_resp_gateway(byte SrcAddr, byte PayloadLen, byte *Payload, boolean *to_send_resp_nowB)
{
...
byte IRMA_M_v20_r_Resp(word PayloadLen, byte *Payload)
{
	...
	for( MemByteIdx = 0; MemByteIdx < MemByteCnt; MemByteIdx++ )
		IRMASendBuff[IRMASendBuffWritePos++] = *(byte *)(MemAddr++);

	return(0);
}	// IRMA_M_v20_r_Resp


byte IRMA_M_v20_v_w_Resp(boolean to_verifyB, word PayloadLen, byte *Payload)
{
...
void irma_msg_version_gateway(byte uip_ver, byte service_lev, boolean retry_flagB, byte SrcAddr, byte DataVer, char sub_cmd, word PayloadLen, byte *Payload)
{
	...
	switch( DataVer )
	{
		...
		default   :	Send_IRMA_Error_v10(SrcAddr, 'V', IRMAErr_UnkTypeIdentVer, 1, (byte *)&DataVer);
					break;
	}
